...
このシステムは、ブラウザ上の 3D 空間に VRM アバターを表示し、`Start Action` ボタンであらかじめ定義した動作シーケンスを再生する教材用アプリケーションです。
...
| 要素 | 内容 |
| --- | --- |
| `<button id="startActionButton">` | 画面下中央に表示される開始ボタンです。クリックすると `startCharacterAction()` が実行され、`lesson_actions.js` のシーケンス再生が始まります。 |
...
`Start Action` は、VRM がまだ読み込まれていない場合やシーケンス再生中の場合は実行されません。
...
window.lessonActionSettings = {
	...
	sequence: [
		...
		{ type: 'speak', message: 'こんにちは。', rate: 1.2 },
...
| `sequence` | キャラクターに実行させる動作の配列です。 | `stepForward`, `bow`, `speak`, `stepBack` |

セリフは各 `speak` ステップに直接 `message`, `rate`, `typeDurationMs`, `waitMode` を指定します。
...
| --- | --- | --- | --- | --- |
| `stepForward` | キャラクターの向いている方向へ少し前進します。 | `distance`, `durationMs` | `distance: 0.55`, `durationMs: 700` | なし |
| `stepBack` | キャラクターの向きと逆方向へ少し後退します。 | `distance`, `durationMs` | `distance: 0.55`, `durationMs: 700` | なし |
...
| `type` | 内容 | 推奨パラメータ | 省略時の値 | VRMA 再生 |
| --- | --- | --- | --- | --- |
| `speak` | セリフを音声読み上げし、頭付近に吹き出しを表示します。 | `message`, `rate`, `typeDurationMs`, `waitMode` | `message: 'こんにちは！'`, `rate: 1.8`, `typeDurationMs: 150`, `waitMode: 1` | `waitMode: 1` のとき `Talking.vrma` をループ再生 |
...
基本アクションのうち、実行中にVRMAを本再生する可能性があるのは `speak` だけです。`speak` でも `waitMode: 0` の場合は `Talking.vrma` を再生しません。`stepForward`, `stepBack`, `bow`, `effect0`, `effect1`, `effect2`, `wait`, `avatar` はプログラム処理で動作します。

`effect0`, `effect1`, `effect2` はA-Frame空間内の3Dオブジェクトとして生成されるため、カメラを動かすと3D空間上の位置関係も変わります。

すべてのアクションの切り替わりでは、前のアクションの最後の姿勢から、次のアクションの最初の姿勢へ `500ms` かけて補間します。`stepForward`, `stepBack`, `bow` の開始姿勢には `Standing Idle` の先頭姿勢を使います。

### 6.2 VRMA アニメーション

VRMA アニメーションは、VRM アバター用のモーションファイル `.vrma` を読み込んで再生するアクションです。基本アクションが `main.js` の計算で位置や姿勢を動かすのに対し、VRMA アニメーションはファイルに記録された全身モーションをアバターのボーンへ適用します。

以下の `type` は VRMA モーションとして扱われます。短く書く場合は `'Walking'` のような文字列で、パラメータを指定する場合は `{ type: 'Walking', ... }` のようなオブジェクトで書きます。
...
| `Slow Run` | `Slow Run.vrma` | 走るモーション | `durationMs`, `speed`, `turnAngle` | `durationMs: 3000`, `speed: 1.8`, `turnAngle: 0` |
...
| `type` | ファイル | 内容 | 推奨パラメータ | 省略時の値 |
| --- | --- | --- | --- | --- |
| `Show Body` | `Show Body.vrma` | 全身を見せるポーズ | `durationMs` | モーションを1回再生 |
| `Greeting` | `Greeting.vrma` | 挨拶モーション | `durationMs` | モーションを1回再生 |
| `Peace Sign` | `Peace Sign.vrma` | Vサイン、ピースサイン | `durationMs` | モーションを1回再生 |
| `Shoot` | `Shoot.vrma` | 撃つポーズ | `durationMs` | モーションを1回再生 |
| `Model Pose` | `Model Pose.vrma` | モデルポーズ | `durationMs` | モーションを1回再生 |
...
| `Squat` | `Squat.vrma` | 屈伸運動 | `durationMs` | モーションを1回再生 |
...
| `Spin towice` | `Spin In Place.vrma` | 2回転するモーション | なし *1 | モーションを1回再生 |
...
`durationMs` は VRMA アニメーションにも指定できます。`Walking`, `Slow Run`, `walkForward`, `walkBack` では移動時間として扱われます。その他の VRMA では次の動作になります。
...
*1 : `Cross Jumps`, `Jump`, `Left Turn`, `Right Turn`, `Spin towice`, `Spin once` は `durationMs` パラメータを持ってはいますが、なるべく `durationMs` を指定しないでください。これらは比較的短いモーション、または開始時と終了時で位置・回転・姿勢の差が大きいモーションです。途中終了や繰り返しを行うと、前後のアクションとの姿勢差分が大きくなり、つなぎ目でキャラクターがちらついて見えやすくなります。ちらつきを防ぐため、これらはモーションを1回そのまま再生する使い方（`durationMs` を未指定）を推奨します。
...
| `speed` | 1 秒あたりの移動距離です。 | `Walking: 0.62`, `Slow Run: 1.8` |
...
| 速度 | PCブラウザの音声読み上げでは `rate` で指定します。内部では `0.1` から `10` の範囲に制限されます。 |
...
`waitMode: 0` のセリフは、指定された `message`、`rate`、`typeDurationMs` に従って完了します。セリフが長い場合は、後続の複数アクションにまたがって吹き出し表示や音声再生が続くことがあります。文字表示と音声再生の両方が終わると、吹き出しは自動的に消えます。
...
| 表示速度 | `typeDurationMs` は1文字表示後に次の文字を出すまでの待ち時間です。省略時は `150ms` です。 |
| 改行 | 15 文字ごとに自動改行します。メッセージ内の `\n` も段落として扱います。 |
| 幅 | 文字数に応じて `320px` から `560px` の範囲で調整されます。 |
| 画面端処理 | 画面外にはみ出さないように位置を補正します。 |

`typeDurationMs` はセリフ全体の表示時間ではありません。例えば `typeDurationMs: 150` の場合、1文字表示したあと約 `150ms` 待って次の文字を表示します。値を小さくすると速く、大きくするとゆっくり表示されます。

| `typeDurationMs` | 表示速度の目安 |
| --- | --- |
| `40` | かなり速い |
| `70` | 速め |
| `100` | 標準的なタイプライター風 |
...
アクションの切り替わりでは、前のアクションの最後の姿勢から、次のアクションの最初の姿勢へ補間します。対象は基本アクションと VRMA アニメーションのすべての組み合わせです。たとえば、基本アクションから VRMA、VRMA から基本アクション、基本アクション同士、VRMA 同士のどの切り替わりでも補間が入ります。
...
反映後は、終了時の見た目を保つように `Hips` の位置と回転を補正します。これにより、`Left Turn`、`Right Turn`、`Spin towice` など、モーション内にルート回転や水平移動を持つ VRMA でも、キャラクター本体と `Hips` に同じ動きが二重に適用されることを防ぎます。
...
1. `index.html` が外部ライブラリ、`lesson_actions.js`、`main.js` を読み込みます。
...
6. `Start Action` ボタンが押されると、`lessonActionSettings.sequence` が正規化されます。
7. 各ステップを順番に実行します。
8. 各ステップ終了時にポーズ補間を行い、次のステップへ進みます。
9. 最後まで終わるとアニメーションを停止し、カメラを見やすい位置へ戻し、待機モーションへ戻ります。
...
| 確認項目 | 期待結果 |
| --- | --- |
| ページ表示 | ブラウザに 3D 空間、統計表示、`Start Action` ボタンが表示される。 |
| アバター表示 | `lesson_actions.js` の `avatar` に対応する VRM が表示される。 |
| 開始ボタン | VRM 読み込み後にクリックすると `sequence` の最初の動作が始まる。 |
| 基本動作 | `stepForward`, `stepBack`, `bow`, `wait`, `effect0`, `effect1`, `effect2`, `avatar` が指定通りに完了する。 |
...
| VRMA時間指定 | 非移動モーションも `durationMs` で短縮または繰り返しできる。ただし `Cross Jumps`, `Jump`, `Left Turn`, `Right Turn`, `Spin towice`, `Spin once` は、ちらつき防止のため `durationMs` を指定しない。 |
...
- `Cross Jumps`, `Jump`, `Left Turn`, `Right Turn`, `Spin towice`, `Spin once` は、開始時と終了時の位置・回転・姿勢差が大きい、または再生時間が短いモーションです。`durationMs` で途中終了や繰り返しを行うと、前後のアクションとの姿勢差分が大きくなり、つなぎ目のちらつきとして目立つ場合があります。これらの type では `durationMs` を指定せず、モーションを1回そのまま再生してください。
...
window.lessonActionSettings = {
	...
	sequence: [
		'stepForward',
		'bow',
		{ type: 'speak', message: 'こんにちは。今日はよろしくお願いします。', rate: 1.2 },
		'stepBack'
	]
};
```
...
window.lessonActionSettings = {
	...
	sequence: [
		...
		{ type: 'speak', message: '少し右に曲がりながら歩きました。' },
		{ type: 'Walking', durationMs: 4000, speed: 0.7, turnAngle: -45 }
	]
};
```
...
window.lessonActionSettings = {
	...
	effectColor: '#7ee7ff',
	sequence: [
		{ type: 'speak', message: 'それでは、少し踊ってみます。' },
...
window.lessonActionSettings = {
	avatar: 0,
	sequence: [
		{ type: 'speak', message: '最初のアバターです。' },
		{ type: 'avatar', avatar: z },
		{ type: 'speak', message: 'アバターを変更しました。' }
	]
};
```
